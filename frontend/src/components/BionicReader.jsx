...
export default function BionicReader() {
  const { text, sourceLabel, fixation, fontFamily, fontSize, lineHeight, readingWidth, clearText } = useReader();
  ...
  return (
    <section
      ...
      className="relative flex-1 min-h-[70vh] bg-[hsl(var(--surface))] border border-border"
    >
      {/* Reader chrome bar */}
      <header className="flex items-center justify-between border-b border-border px-6 py-3">
        <div className="flex items-center gap-4">
          <span className="label-caps">Reader</span>
          {sourceLabel ? (
            <span
              data-testid="reader-source-label"
              className="text-xs text-muted-foreground font-mono truncate max-w-[40ch]"
            >
              ← {sourceLabel}
            </span>
          ) : null}
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              className="rounded-none h-8 hover:text-[hsl(var(--accent))]"
            >
      ...
      {/* Reader body */}
      <div className="relative paper-grain">
        {text ? (
          <div
            className="mx-auto px-6 md:px-10 lg:px-14 py-10 md:py-14"
            style={{ maxWidth: `${readingWidth}ch` }}
          >
            <article
...
function EmptyState() {
  return (
    <div
      data-testid="reader-empty-state"
      className="min-h-[60vh] flex flex-col items-center justify-center px-8 py-20 text-center"
    >
      <div className="label-caps mb-6">Awaiting Input</div>
      <h2
        className="text-3xl md:text-5xl tracking-tight font-light max-w-[24ch]"
      ...
      <p className="mt-8 text-sm text-muted-foreground max-w-[42ch]">
        Paste text, drop a PDF, upload an image or paste a URL —
        every word gets a fixation anchor for your eye to lock onto.
      </p>
